...
#Damon Beam - Python Project 2 - Expected time 3 hours - Actual time: 3 hours
#Expected time met actual time. I struggled a little bit with the classes at first but got something down that I could make use of. Overall, I'm happy with where it is at now but with
#more time I could definitely make it make more natural/complex sentences.
...
for Word.word, WordStructure.partsofspeech in wordList.tags:
    if WordStructure.partsofspeech == 'JJ':
        adjList.append(Word.word)

verbList = list()
for Word.word, WordStructure.partsofspeech in wordList.tags:
    if WordStructure.partsofspeech == 'VB':
        verbList.append(Word.word.pluralize())

advList = list()
for Word.word, WordStructure.partsofspeech in wordList.tags:
    if WordStructure.partsofspeech == 'RB':
...
nounList = list()
for Word.word, WordStructure.partsofspeech in wordList.tags:
    if WordStructure.partsofspeech == 'NN':
...
def three_word_sentence():
    ...
    print(Sentence.subject + Sentence.predicate)
